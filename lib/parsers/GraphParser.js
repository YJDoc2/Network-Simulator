...
const TranslateGraphCordinates = (network_config) => {
	let network = new window.vis.Network(
		document.getElementById("network-visjs"), // div container that points to a div which is outside the screen
		{ nodes: network_config.parsed_nodes, edges: network_config.parsed_edges }, // nodes and edges
		{
			...
			height: `${document.getElementById("network-svg").clientHeight}px`,
			width: `${document.getElementById("network-svg").clientWidth}px`,
		} // options
	);
