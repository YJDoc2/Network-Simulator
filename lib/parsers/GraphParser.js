...
const TranslateGraphCordinates = (network_config) => {
	...
	let network = new window.vis.Network(
		document.getElementById("network"), // div container
		{ nodes: network_config.parsed_nodes, edges: network_config.parsed_edges }, // nodes and edges
		{
			nodes: { fixed: true },
			layout: { randomSeed: 0 },
			height: `${document.getElementById("network").clientHeight}px`,
			width: `${document.getElementById("network").clientWidth}px`,
		} // options
	);
