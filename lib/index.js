...
exports.getParsedInputGraph = (input) =>{
    ...
    { 
      let first=edges[i][0];
      let second=edges[i][2];
      
      if(!nodes.has(first))
      {
        nodes.set(first,count);
        count=count+1;
      }
      if(!nodes.has(second))
      {
        nodes.set(second,count);
        count=count+1;
      }
    }
    for(let i=0;i<edges.length;i++)
    {
      let first=edges[i][0];
      let second=edges[i][2];
      parsed_input_edges.push({from:nodes.get(first),to:nodes.get(second)});
    }
    for (let [key, value] of nodes) {
      
      parsed_input_nodes.push({id:value,label:key});
    }
    console.log(parsed_input_nodes);
    console.log(parsed_input_edges);
    let obj={
