    ...
    this.nodes.forEach((node) => {
      ...
      node.queue.forEach((packet) => {
        ...
        try {
          ...
          forwards = node.fn(node.mem, packet.packet);
          if (node.mem !== JSON.parse(node.memStr)) {
        ...
        forwards.forEach((p) => {
          ...
          if (p.to === FLOOD) {
            ...
            if (packet.from === ENQUEUED) {
              ...
              global_queue.push(
                this.createPacket(node.name, packet.from, color, p)
              );
            }
          } else if (node.neighbors.has(p.to)) {
            // make sure that node has the destination connected to it
            global_queue.push(this.createPacket(node.name, p.to, color, p));
          } else {
            Logger.log(
              `Error in processing Packet #${packet.id} at node ${
                node.name
